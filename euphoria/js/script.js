...
if (mainProduct) {
	...
	if (mainProductSliderImages.length) {
		...
		mainProductThumbSliderTemplate += `
			<button type="button" class="thumb-slider__arrow thumb-slider__arrow--up _icon-chevron_up"></button>
			<button type="button" class="thumb-slider__arrow thumb-slider__arrow--down _icon-chevron_down"></button>
		`
		mainProductThumbSliderTemplate += `</div>`
		mainProductThumbSliderTemplate += `</div>`
...
if (spollers.length) {
	...
	// // Filter
	const filterTitel = document.querySelector('.filter__titel')
	if (filterTitel) {
		//window.addEventListener('resize', someFunc);
		const breakPointValue = filterTitel.dataset.spollerMedia;
		...
		if (breakPoint) {
			...
			matchMedia.addEventListener("change", (e) => {
				...
				if (isTrue) {
					slideUp(filterTitel.nextElementSibling)
					filterTitel.classList.remove('active')
				} else {
					slideDown(filterTitel.nextElementSibling)
					filterTitel.classList.add('active')
				}
			})
		}
	}
...
// // Price-Готове рішення без підключення wNumb
...
function initProducts(data) {
	...
	if (productsList.length) {
		...
		productsList.forEach(productItem => {
			...
			productTemplate += `<a href="#" class="item-product__favorite _icon-heart" ${productItem.favorite ? 'item-product__favorite--active' : null} _icon-favorite"></a>`
			if (productItem.image) {
				productTemplate += `<a href="${productItem.url}" class="item-product__picture-link">`
				productTemplate += `<img src="${productItem.image}" class="item-product__img" alt="Image">`
				productTemplate += `</a>`
			}
